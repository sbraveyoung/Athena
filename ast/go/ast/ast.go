...
type (
	...
	// A variable node represents a variable like ${x}
	Variable struct {
		StartPos token.Pos
		Name     string
	}
...
func (x *Variable) Pos() token.Pos { return x.StartPos }
...
func (x *Variable) End() token.Pos { return token.Pos(int(x.StartPos) + len(x.Name)) }
